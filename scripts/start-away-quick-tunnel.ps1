...
"@
    ...
    [void][CodexInPhoneQuickTunnelPower]::SetThreadExecutionState($ES_CONTINUOUS -bor $ES_SYSTEM_REQUIRED -bor $ES_AWAYMODE_REQUIRED)
}

function Disable-KeepAwake {
    if ("CodexInPhoneQuickTunnelPower" -as [type]) {
        [void][CodexInPhoneQuickTunnelPower]::SetThreadExecutionState(0x80000000)
    }
}
